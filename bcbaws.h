...
//};
  ...
  state.json = response;
  writeFile(SD, "/data.json", state.json.c_str());
  return response;
}
...
void updateDB(){
      ...
      strcpy(req, ("http://192.168.1.177/~brian/esp32/mysql-data-logger/weather.php?temp=" + String(state.temp) + "&press=" + String(state.pressure) + "&p64=" + String(state.pressure64) + "&humid=" + String(state.humidity)).c_str());
